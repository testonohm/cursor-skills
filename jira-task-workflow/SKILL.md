...
description: >-
  ...
  context via Atlassian MCP, determine target repository from Jira labels,
  align scope and acceptance criteria, branch and implement in that repo,
  verify, open PR, update Jira status and comments. If labels do not identify
  exactly one repo, the agent must ask the user every time. Use when building
  or fixing work that begins from a Jira ticket, issue key (e.g. PROJ-123),
  Jira URL, or phrases like "จาก Jira", "งาน SD-", "เริ่มจาก task บน Jira".
...
2. Resolve the **target repository** using **Labels** only (see next section). Never infer repo from summary, title, or description alone.
3. List **assumptions**, **open questions**, and a **Definition of Done** (tests, docs, feature flags, rollout) before writing code. For topics other than repo selection, stop and ask only when delivery is blocked by missing product or access decisions.
4. If the issue is a parent with sub-tasks, confirm whether this session covers the parent only, one sub-task, or the full breakdown.
...
- Empty description: pull latest comments; ask the reporter/po for acceptance criteria in Jira before large implementation.
